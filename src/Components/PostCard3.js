...
import './PostCard.css';
import { FiUsersAdd } from 'react-icons/fi';
import { FaCalendarAlt, FaMapMarkerAlt } from 'react-icons/fa';
import { FaEye, FaShare, FaEllipsisH } from 'react-icons/fa';
import { IoMdShare } from 'react-icons/io';
import { GiSuitcase } from 'react-icons/gi';

const PostCard = ({  theme, content, userName, views, userProfileImageUrl }) => {
    ...
    <div className="post-card-wrapper">
      <div className="post-card">
        ...
        <div className="post-content">
          <h4 style={{ fontStyle: 'italic' }}>{theme}</h4>
         <h4 className='justify-content-start'><strong>{content}</strong></h4>

         <div className="dropdown">
         <button className="ellipsis-icon float-end" onClick={toggleOptions} style={{ background: 'none' }}>
  <FaEllipsisH />
</button>
          ...
          <div className="date-location">
            <div className='d-flex align-items-center'>
             <span className="ms-2"><GiSuitcase style={{ color:'chocolate',fontSize:'25px'}}/>Innovaccer Analytics Private Ltd.</span>
              <center style={{ marginLeft : "400px"}}><FaMapMarkerAlt className="me-1 " />Ahmedabad, India</center>
            </div>
          </div>
          <div className="mt-5 ml-5">

            <button className='btn btn-lg text-success text-center border border-dark' style={{ width: '60vw', background: 'none', borderRadius: '20px' }}>Visit Website</button>
          </div>
